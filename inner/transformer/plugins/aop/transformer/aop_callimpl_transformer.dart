...
class AopCallImplTransformer extends Transformer {
  ...
  StaticInvocation visitStaticInvocation(StaticInvocation staticInvocation) {
    ...
    if (node == null) {
      ...
      final String? procedureName = canonicalName?.name;
      String? tempName = canonicalName?.parent?.parent?.name;
      if (tempName == '@methods') {
        tempName = canonicalName?.parent?.parent?.parent?.name;
      }
      //Library Static
      ...
      // Class Static
        ...
        tempName = canonicalName?.parent?.parent?.parent?.name;
        final String? clsName = canonicalName?.parent?.parent?.name;
        final Library? originalLibrary = _libraryMap[tempName];
...
//      assert(false);
  ...
  InstanceInvocation visitInstanceInvocation(
    ...
    if (node is Procedure || node == null) {
      if (node is Procedure) {
        ...
        importUri = canonicalName?.reference?.canonicalName?.nonRootTop?.name;
        clsName = canonicalName?.parent?.parent?.name;
        methodName = canonicalName?.name;
      }
      AopItemInfo? aopItemInfo = _filterAopItemInfo(
  ...
  InstanceInvocation transformInstanceMethodInvocation(
    ...
    Class? methodClass = methodProcedure.parent as Class?;

    Class? methodImplClass = methodClass;
    final String procedureName = instanceInvocation.name.text;
    Library? originalLibrary = methodClass?.parent as Library?;
    if (originalLibrary == null) {
      final CanonicalName? canonicalName =
          instanceInvocation.interfaceTargetReference.canonicalName;
      final String? libImportUri = canonicalName?.nonRootTop?.name;
      originalLibrary = _libraryMap[libImportUri];
    }
    if (methodClass == null) {
      final CanonicalName? canonicalName =
          instanceInvocation.interfaceTargetReference.canonicalName;
      final String? expectedName = canonicalName?.parent?.parent?.name;
      for (Class cls in originalLibrary!.classes) {
    ...
    if (methodClass != null && methodClass.flags & Class.FlagAbstract != 0) {
      for (Class cls in originalLibrary!.classes) {
